...
function Valid() {
  ...
  const [widths, setWidths] = useState(widthsFromScores(scores));
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);
  const [chatHistory, setChatHistory] = useState([]);
  const [chatInput, setChatInput] = useState("");
  const [chatLoading, setChatLoading] = useState(false);
  const [gaps, setGaps] = useState(miss?.length || 0);
  const [activeTab, setActiveTab] = useState(
    ideaComplete ? "report" : "scores"
  );
  const [showExamples, setShowExamples] = useState(!ideaComplete);
  ...
  function buildReport(fr) {
    if (!fr) return null;
    return {
      idea: fr.complete_idea?.title,
      complete_idea: fr.complete_idea,
      final_scores: fr.final_scores,
      market_viability: fr.market_viability,
      strengths: fr.strengths,
      assumptions: fr.assumptions,
      remaining_gaps: fr.remaining_gaps,
      uvp: {
        statement: fr.complete_idea?.competitive_advantage,
        points: [
          { title: "Problem", desc: fr.complete_idea?.problem },
          { title: "Solution", desc: fr.complete_idea?.solution },
          { title: "Revenue", desc: fr.complete_idea?.revenue_model },
        ]
      },
      takeaways: fr.strengths?.map(s => ({
        title: s.dimension?.replace(/_/g, " "),
        desc: s.finding
      })) || [],
      next_steps: fr.market_viability?.reasoning || ""
    };
  }

  function handleIdeaComplete(fr) {
    setReply((prev) => ({
      ...prev,
      scores: fr.final_scores,
    }));
    setOverall(fr.overall_score);
    const filled = {};
    Object.entries(fr.final_scores).forEach(([k, v]) => {
      filled[k] = v.score || 0;
    });
    setWidths(filled);
    const builtReport = buildReport(fr);
    setReport(builtReport);
    setMiss(fr.remaining_gaps || []);
    setGaps(fr.remaining_gaps?.length || 0);
    setActiveTab("report");
    setCompleteIdea(fr.complete_idea);
    setFinalResult(fr);
    setOverall(fr.overall_score);
    setScores(fr.final_scores);
    setIdea({msg:"complete"})
  }
  ...
  try {
    ...
    // ✅ Fixed
    setReply({ scores: data.analysis });
    setOverall(data.overall);
    setMiss(data.gaps || []);
    setGaps(data.gaps?.length || 0);
    ...
    if (data.status === "IDEA_COMPLETE" && data.finalResult) {
      handleIdeaComplete(data.finalResult);
          ...
          <div style={{
            ...
            <div style={{
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "24px" }}>
                ...
                {/* Tabs */}
                <div style={{ display: "flex", gap: "8px", background: "rgba(0,0,0,0.4)", padding: "4px", borderRadius: "12px", flexWrap: "wrap" }}>
                  {[
                ...
                <div style={{
                  ...
                  <p style={{ color: "#cbd5e1", fontSize: "14px", lineHeight: "1.6" }}>
                    {overall >= 70 ?
                      ...
                      "Consider pivoting or significantly refining your approach. Use the chat feature to explore alternative angles."}
                  </p>
                </div>
              </div>
            )}
              ...
              <div style={{ padding: "32px" }}>
                {miss.length === 0 ? (
                  <div style={{ textAlign: "center", padding: "40px", color: "#94a3b8" }}>
                    ...
                    <p style={{ fontSize: "16px", color: "white", fontWeight: "600", marginBottom: "8px" }}>
                      All gaps resolved!
                    </p>
                    <p style={{ fontSize: "14px" }}>
                      {ideaComplete
                        ? "Your idea is fully validated. Check the Full Report tab."
                        : "No critical gaps found! Your idea is well defined."}
                    </p>
                  </div>
                ) : (
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(350px, 1fr))", gap: "20px" }}>
                    {miss.map((gap, i) => (
                      ...
                      >
                        ...
                        <div style={{ padding: "10px 12px", background: "rgba(0,0,0,0.3)", borderRadius: "8px", marginBottom: "8px" }}>
                        ...
                        <div style={{ padding: "10px 12px", background: "rgba(0,0,0,0.3)", borderRadius: "8px" }}>
              ...
              <div style={{ padding: "32px", display: "flex", flexDirection: "column", height: "600px" }}>
                <div style={{ flex: 1, overflowY: "auto", marginBottom: "20px", display: "flex", flexDirection: "column", gap: "12px" }}>
                  {chatHistory.length === 0 && (
                    <div style={{ textAlign: "center", padding: "40px", color: "#94a3b8" }}>
                      <span style={{ fontSize: "48px", display: "block", marginBottom: "16px" }}>💬</span>
                      <p>Start a conversation with Alex to refine your idea</p>
                    </div>
                  )}
                  {chatHistory.map((msg, i) => (
                    <div key={i} style={{ display: "flex", justifyContent: msg.role === "founder" ? "flex-end" : "flex-start" }}>
                      <div style={{
                        ...
                        fontSize: "14px",
                        lineHeight: "1.5",
                        background: msg.role === "founder" ? "linear-gradient(135deg, #8b5cf6, #6366f1)" : "rgba(255,255,255,0.06)",
                      ...
                      <div style={{
                        ...
                        <div>
                          ...
                          <div style={{ fontSize: "14px", color: "#94a3b8" }}>
                            Ready to see how you stand out from competitors?
                          </div>
                        </div>
                        <button
